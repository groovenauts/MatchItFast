  ...
  useEffect(() => {
    async function getOgpImage(url: string) {
        if (process.env.NODE_ENV === "production") {
            const res = await fetch("/api/ogp_image", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ "url": url}) });
            if (res.status !== 200) {
              console.log("/api/ogp_image return HTTP status: " + res.status);
            } else {
                const result = await res.json();
                return result["ogpImage"];
            }
            return null;
        } else {
    ...
    if (neighbors === null) {
        ...
        const f = async() => {
            for (let i = 0; i < neighbors.length; i++) {
                if (neighbors[i].ogpImage === undefined) {
                    const ns = neighbors.slice(0, neighbors.length);
                    const ogpImage = await getOgpImage(neighbors[i].url);
                    ns[i].ogpImage = ogpImage;
                    setNeighbors(ns);
                    return;
                }
            }
